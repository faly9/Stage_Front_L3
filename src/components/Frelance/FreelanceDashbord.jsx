...
useEffect(() => {
    ...
    switch (action) {
      case "created": {
        ...
        try {
          ...
          console.error("❌ Erreur fetch missions après création :", err.message);
        }
        break;
      }
  ...
  const postuler = (mission) => {
    ...
    const ws = new WebSocket(`ws://localhost:8001/ws/candidatures/${entrepriseId}/`);

    ws.onopen = () => {
      console.log("✅ WebSocket Candidature connecté à l'entreprise", entrepriseId);
    ...
    ws.onmessage = (event) => {
      ...
      if (data.error) {
        ...
        toast.success(`🎉 Candidature envoyée pour la mission : ${mission.titre}`);
      }
